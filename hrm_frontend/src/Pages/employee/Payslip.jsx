    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          {[
            { label: "Last Salary",   value: `Rs. ${lastNet.toLocaleString()}`, icon: "💳", gradient: "from-sky-400 to-blue-500",    sub: payslips.length > 0 ? `${getMonthName(payslips[0].month)} ${payslips[0].year}` : "N/A" },
            { label: "Avg. Monthly",  value: `Rs. ${avgNet.toLocaleString()}`, icon: "📈", gradient: "from-emerald-400 to-teal-500", sub: "All time" },
            { label: "Total Earnings",  value: `Rs. ${totalNet.toLocaleString()}`, icon: "💰", gradient: "from-violet-400 to-purple-500", sub: "Year to date" },
              ...
              <div key={p.id} className="border border-gray-100 rounded-xl p-5 hover:border-sky-200 hover:shadow-sm transition-all">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-800">{getMonthName(p.month)} {p.year}</h3>
                      <p className="text-sm text-gray-500">Status: {p.status || "PAID"}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm flex-1 md:ml-6 bg-gray-50 rounded-xl p-4">
                    ...
                    <div><p className="text-gray-400 text-xs mb-0.5">Allowances</p><p className="font-bold text-emerald-600">+{p.totalAllowances.toLocaleString()}</p></div>
                    <div><p className="text-gray-400 text-xs mb-0.5">Deductions</p><p className="font-bold text-red-500">-{p.totalDeductions.toLocaleString()}</p></div>
            ...
            <div className="p-6 space-y-5">
              <div className="bg-gray-50 rounded-xl p-4 grid grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  <div key={l}><p className="text-gray-400 text-xs">{l}</p><p className="font-semibold text-gray-800">{v || "N/A"}</p></div>
                ))}
              </div>
              <div>
                <h3 className="font-bold text-gray-800 mb-3">Earnings & Allowances</h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Basic Salary</span><span className="font-semibold">Rs. {viewingPayslip.basicSalary.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Other Allowances</span><span className="font-semibold">Rs. {viewingPayslip.totalAllowances.toLocaleString()}</span>
                  </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Total Deductions</span><span className="font-semibold">Rs. {viewingPayslip.totalDeductions.toLocaleString()}</span>
                  </div>
