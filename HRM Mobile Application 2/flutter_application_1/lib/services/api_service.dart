...
class ApiService {
  ...
  static const String authBaseUrl = 'http://127.0.0.1:5002/api';     // User_Backend
  static const String employeeBaseUrl = 'http://127.0.0.1:5003/api'; // Employee_Backend
  ...
  static Future<Map<String, dynamic>> getLeaveBalances() async {
    final url = Uri.parse('$employeeBaseUrl/employee/leaves/balance');
    final response = await http.get(url, headers: _headers());
    return _handleResponse(response);
  }
  ...
  static Future<Map<String, dynamic>> applyLeave(int leaveTypeId, String startDate, String endDate, String reason) async {
    final url = Uri.parse('$employeeBaseUrl/employee/leaves/apply');
    final response = await http.post(
      url,
      headers: _headers(),
      body: json.encode({
  ...
  static Future<Map<String, dynamic>> getMyLeaves() async {
    final url = Uri.parse('$employeeBaseUrl/employee/leaves/my-leaves');
    final response = await http.get(url, headers: _headers());
    return _handleResponse(response);
  }
  ...
  static Future<Map<String, dynamic>> cancelLeave(int leaveId) async {
    final url = Uri.parse('$employeeBaseUrl/employee/leaves/$leaveId/cancel');
